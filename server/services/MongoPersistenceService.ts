...
export class MongoPersistenceService {
    ...
    /**
     * Get a raw MongoDB collection for direct access
     * Used by services that need custom data storage
     */
    getCollection(name: string) {
        if (!this.isReady()) {
            return null;
        }
        return database.getConnection().collection(name);
    }
    ...
    async getGlobalStats(): Promise<any> {
        try {
            const [
                ...
                Promise.resolve(0), // Star count (static assets, not fully in DB yet unless lit)
                LitStar.countDocuments(),
                Friendship.countDocuments()
            ]);

            const recentPlayers = await Player.countDocuments({
            ...
            return {
                players: {
                    total: playerCount,
                    active24h: recentPlayers
                },
                content: {
                    echoes: echoCount,
                    messages: messageCount,
                    litStars: litStarCount,
