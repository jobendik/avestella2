...
export function PulseButton(): JSX.Element {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      {currentPattern && (
        <div className={`absolute bg-gradient-to-r from-purple-600 to-pink-500 text-white px-3 py-1.5 rounded-xl border border-white/30 shadow-lg animate-bounce ${isMobile ? '-top-12 text-xs' : '-top-16 text-sm'}`}>
      ...
      <button
        ...
        className={`${isMobile ? 'w-12 h-12' : 'w-14 h-14'} rounded-2xl flex items-center justify-center border transition-all duration-300 relative group ${isPulsing
      ...
      >
        <Heart size={isMobile ? 20 : 24} fill={isPulsing ? "currentColor" : "none"} className={isPulsing ? 'animate-ping' : ''} />
        <div className="absolute -top-1 -right-1 w-2 h-2 bg-green-500 rounded-full animate-pulse shadow-[0_0_5px_#22c55e]" />
      </button>

      {/* Label */}
      <div className="md:block hidden text-[9px] text-white/40 tracking-widest font-medium">PULSE</div>
      <div className="md:block hidden text-[8px] text-white/30 max-w-[80px] text-center">
